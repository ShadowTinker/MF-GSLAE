...
class MacridDAE30(CrossDomainRecommender, AutoEncoderMixin):
    r"""(27) + learnbale mask
    ...
    def __init__(self, config, dataset):
        super(MacridDAE30, self).__init__(config, dataset)
    ...
    def get_cates(self, cates_logits):
        if self.nogb:
            cates = torch.softmax(cates_logits, dim=0)
        else:
            cates_sample = F.gumbel_softmax(cates_logits, tau=0.1, hard=False, dim=0)
            cates_mode = torch.softmax(cates_logits, dim=0)
