...
class MacridDAE38(CrossDomainRecommender, AutoEncoderMixin):
    r"""(30) learnbale mask + reconstruct item-item matrix

    """
    input_type = InputType.PAIRWISE

    def __init__(self, config, dataset):
        super(MacridDAE38, self).__init__(config, dataset)
        ...
        self.item_decoder = nn.Embedding(self.total_num_items, self.lat_dim)
        ...
        if config['fix_mask']:
            self.mask_source.weight.requires_grad_(False)
            self.mask_target.weight.requires_grad_(False)
            self.mask_source.weight[:, :self.kfac // 2] = 0.5
            self.mask_source.weight[:, self.kfac // 2:] = -0.5
            self.mask_target.weight[:, :self.kfac // 2] = -0.5
            self.mask_target.weight[:, self.kfac // 2:] = 0.5
    ...
    def forward(self, source_user=None, target_user=None):
        ...
        items_graph = F.normalize(self.item_decoder.weight, dim=-1)
        ...
        if source_user != None:
            ...
            for k in range(self.kfac):
                ...
                # ================ item item input ==============
                item_item_k = x_k_source.T @ x_k_source
                h_source = self.encoder(item_item_k)
                h_source, graph = self.transformer_layer(F.normalize(h_source))
                z_k_source = F.normalize(h_source)
                logits_k = torch.matmul(z_k_source, items_graph.T)
                self.source_rec_loss += -(F.log_softmax(logits_k, 1) * item_item_k).sum(1).mean()
        ...
        if target_user != None:
            ...
            for k in range(self.kfac):
                ...
                # ================ item item input ==============
                item_item_k = x_k_target.T @ x_k_target
                h_target = self.encoder(item_item_k)
                h_target, graph = self.transformer_layer(F.normalize(h_target))
                z_k_target = F.normalize(h_target)
                logits_k = torch.matmul(z_k_target, items_graph.T)
                self.target_rec_loss += -(F.log_softmax(logits_k, 1) * item_item_k).sum(1).mean()
    ...
    def calculate_loss(self, interaction):
        ...
        rec_loss = self.source_rec_loss + self.target_rec_loss

        return (ce_loss_source, ce_loss_target, 1e-2 * rec_loss)
