...
class MacridDAE13(CrossDomainRecommender, AutoEncoderMixin):
    r"""FFT
    ...
    def __init__(self, config, dataset):
        super(MacridDAE13, self).__init__(config, dataset)
    ...
    def forward(self, rating_matrix_source, rating_matrix_target, user=None):
        self.h_list = []
        ...
        for k in range(self.kfac):
            ...
            # z_k_source, z_k_target = self.factor_selection(z_k)
            ...
            h_source = F.normalize(self.encoder_source(x_k_source))
            # z_k_source = z_k_source + F.normalize(h_source)
            ...
            h_target = F.normalize(self.encoder_target(x_k_target))
            # z_k_target = z_k_target + F.normalize(h_target)
        ...
        # probs = torch.cat([probs_target, probs_source], dim=-1)
    ...
    def calculate_loss(self, interaction):
        ...
        rating_matrix_source = self.rating_matrix[source_user_id.cpu()].to(self.device)
        rating_matrix_target = self.rating_matrix[target_user_id.cpu()].to(self.device)

        z_source, z_target = self.forward(rating_matrix_source, rating_matrix_target, all_user_id)
        ...
        ce_loss_source = -(F.log_softmax(z_source, 1) * rating_matrix_source[:, self.target_num_items - 1:]).sum(1).mean()
        ce_loss_target = -(F.log_softmax(z_target, 1) * rating_matrix_target[:, :self.target_num_items]).sum(1).mean()
    ...
    def full_sort_predict_source(self, interaction):
        ...
        scores, _ = self.forward(rating_matrix, self.rating_matrix[[0, 1]].to(self.device))
    ...
    def full_sort_predict_target(self, interaction):
        ...
        _, scores  = self.forward(self.rating_matrix[[0, 1]].to(self.device), rating_matrix)
