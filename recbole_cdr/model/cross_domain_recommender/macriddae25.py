...
class MacridDAE25(CrossDomainRecommender, AutoEncoderMixin):
    r"""FFT + loss calculation (18) -> representation then decomposition

    """
    input_type = InputType.PAIRWISE

    def __init__(self, config, dataset):
        super(MacridDAE25, self).__init__(config, dataset)
        ...
        self.k_embedding_user = nn.Embedding(self.kfac, self.lat_dim)
        ...
        self.intent_adaption = nn.Linear(self.total_num_items, self.lat_dim)
    ...
    def multi_view_lap(self, graph_list):
        lap_val_list = []
        for g in graph_list:
            # lap_val, _ = torch.linalg.eig(g)
            # _, lap_val, _ = torch.svd_lowrank(g)
            lap_val, _ = torch.lobpcg(g, 6, niter=5)
            lap_val_list.append(lap_val)
        mean_g = torch.stack(graph_list, dim=1).mean(1)
        # lap_mean, _ = torch.linalg.eig(mean_g)
        # _, lap_mean, _ = torch.svd_lowrank(mean_g)
        lap_mean, _ = torch.lobpcg(mean_g, 6, niter=5)
        loss = -torch.norm(torch.stack(lap_val_list, dim=0) - lap_mean.unsqueeze(0), p=2, dim=1).sum()
        return loss

    def delta_con(self, graph_list):
        epsilon = self.epsilon
        N = graph_list[0].shape[0]
        def propogation(graph):
            rst = torch.eye(N, device=self.device)
            rst = rst + epsilon * graph + 1e-8
            return rst
        S_list = []
        for g in graph_list:
            S_list.append(torch.sqrt(propogation(g)))
        S_vec = torch.stack(S_list, dim=0).unsqueeze(1)
        loss = -torch.log((S_vec - S_vec.permute(1, 0, 2, 3)).pow(2).sum())
        return loss
    ...
    def forward(self, source_user=None, target_user=None):
        ...
        cores_user = F.normalize(self.k_embedding_user.weight, dim=-1)
        ...
        # cates_logits = self.intent_adaption(cates_logits.T).T

        # cates = self.get_cates(cates_logits).T
        ...
        if source_user != None:
            ...
            users = rating_matrix_source @ items
            cates_logits = ((users @ cores_user.T).unsqueeze(1) * (items @ cores.T).unsqueeze(0)) / self.tau
            self.var_loss += torch.var(cates_logits, dim=0).sum() # control variance of different user

            # cates_logits = (items @ cores.T) / self.tau
        ...
        if target_user != None:
            ...
            users = F.normalize(rating_matrix_target @ items)
            cates_logits = ((users @ cores_user.T).unsqueeze(1) * (items @ cores.T).unsqueeze(0)) / self.tau
            self.var_loss += torch.var(cates_logits, dim=0).sum() # control variance of user intent
            ...
            for k in range(self.kfac):
                ...
                # x_k_target = rating_matrix_target
    ...
    def calculate_loss(self, interaction):
        ...
        # reg_loss += self.multi_view_lap(self.graph_list_source)
        # reg_loss += self.multi_view_lap(self.graph_list_target)
        reg_loss += self.delta_con(self.graph_list_source)
        reg_loss += self.delta_con(self.graph_list_target)
        # reg_loss = torch.tensor(0., device=self.device)
        ...
        return (ce_loss_source, ce_loss_target, self.lmd_disen * compactness_loss, self.lmd_disen * reg_loss, 1e-5 * self.var_loss)
