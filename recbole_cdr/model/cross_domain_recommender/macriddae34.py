...
class MacridDAE34(CrossDomainRecommender, AutoEncoderMixin):
    r"""(30) learnbale mask + reconstruction-induced disentanglement

    """
    input_type = InputType.PAIRWISE

    def __init__(self, config, dataset):
        super(MacridDAE34, self).__init__(config, dataset)
        ...
        # init_mask_source = torch.concat([torch.ones(2, device=self.device), -torch.ones(2, device=self.device)])
        # init_mask_target = torch.concat([-torch.ones(2, device=self.device), torch.ones(2, device=self.device)])
        # self.mask_source.weight.data.copy_(0.3 * init_mask_source)
        # self.mask_target.weight.data.copy_(0.3 * init_mask_target)
    ...
    def forward(self, source_user=None, target_user=None):
        ...
        if source_user != None:
            ...
            for k in range(self.kfac):
                ...
                # decoder for target
                logits_k = torch.matmul(z_k_source, items_target.transpose(0, 1)) / self.tau
                probs_k = torch.exp(logits_k)
                probs_source_target.append(probs_k)
            ...
            if self.if_mask:
                ...
                probs_source_target = (torch.stack(probs_source_target, dim=0) * mask_source).sum(0)
            else:
                probs_source = torch.stack(probs_source, dim=0).sum(0)
            logits_source = torch.log(probs_source + 1e-12)
            logits_source_target = torch.log(probs_source_target + 1e-12)
        else:
            logits_source = None
            logits_source_target = None
        ...
        if target_user != None:
            ...
            for k in range(self.kfac):
                ...
                # decoder for source
                logits_k = torch.matmul(z_k_target, items_source.transpose(0, 1)) / self.tau
                probs_k = torch.exp(logits_k)
                probs_target_source.append(probs_k)
            ...
            if self.if_mask:
                ...
                probs_target_source = (torch.stack(probs_target_source, dim=0) * mask_target).sum(0)
            else:
                probs_target = torch.stack(probs_target, dim=0).sum(0)
            logits_target = torch.log(probs_target + 1e-12)
            logits_target_source = torch.log(probs_target_source + 1e-12)
        else:
            logits_target = None
            logits_target_source = None

        return logits_source, logits_target, logits_source_target, logits_target_source
    ...
    def calculate_loss(self, interaction):
        ...
        rating_matrix_source_target_label = self.rating_matrix_target[source_user_id].to(self.device)
        rating_matrix_target_source_label = self.rating_matrix_source[target_user_id].to(self.device)

        z_source, z_target, z_source_target, z_target_source = self.forward(source_user_id, target_user_id)
        ...
        # Adv CE loss
        adv_loss_source = (F.log_softmax(z_source_target, 1) * rating_matrix_source_target_label).sum(1).mean()
        adv_loss_target = (F.log_softmax(z_target_source, 1) * rating_matrix_target_source_label).sum(1).mean()
        adv_loss = adv_loss_source + adv_loss_target
        ...
        return (ce_loss_source, ce_loss_target, self.lmd_disen * compactness_loss, 0e-3 * mask_reg, 0.1 * adv_loss)

    def full_sort_predict_source(self, interaction):
        user = interaction[self.SOURCE_USER_ID]
        scores, _, _, _ = self.forward(source_user=user.cpu())

        return scores.reshape(-1)

    def full_sort_predict_target(self, interaction):
        user = interaction[self.TARGET_USER_ID]
        _, scores, _, _  = self.forward(target_user=user.cpu())
