...
class BPR_Plus(CrossDomainRecommender):
    ...
    def __init__(self, config, dataset):
        super(BPR_Plus, self).__init__(config, dataset)
        ...
        self.other_parameter_name = ['source_restore_user_e', 'source_restore_item_e', 'target_restore_user_e', 'target_restore_item_e']
                # My layers
        self.proj_source = nn.Linear(self.latent_dim, self.latent_dim)
        self.proj_target = nn.Linear(self.latent_dim, self.latent_dim)
        # self.mask_source = nn.Parameter(torch.empty(1, self.latent_dim))
        # self.mask_target = nn.Parameter(torch.empty(1, self.latent_dim))
    ...
    def forward(self):
        ...
        user_emb_source, user_emb_target = self.factor_selection(user_all_embeddings)
        item_emb_source, item_emb_target = self.factor_selection(item_all_embeddings)

        return user_emb_source, item_emb_source, user_emb_target, item_emb_target
