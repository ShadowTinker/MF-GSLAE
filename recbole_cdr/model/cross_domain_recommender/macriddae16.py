...
class MacridDAE16(CrossDomainRecommender, AutoEncoderMixin):
    r"""bad
    ...
    def __init__(self, config, dataset):
        super(MacridDAE16, self).__init__(config, dataset)
        ...
        self.fac_linear = nn.ModuleList([nn.Linear(self.lat_dim, self.lat_dim) for _ in range(self.kfac)])
    ...
    def forward(self, source_user=None, target_user=None):
        ...
        if source_user != None:
            ...
            probs_source = None
            cates_logits = rating_matrix_source @ items @ cores.T / self.tau
            ...
            for k in range(self.kfac):
                ...
                # h_source, _ = self.factor_selection(h_source)
                z_k_source = F.normalize(h_source)
                z_k_source = z_k_source * cates_k
        ...
        if target_user != None:
            ...
            probs_target = None
            cates_logits = rating_matrix_target @ items @ cores.T / self.tau
            cates = self.get_cates(cates_logits)
            for k in range(self.kfac):
                cates_k = cates[:, k].reshape(-1, 1)
