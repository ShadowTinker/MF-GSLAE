...
class MacridDAE9(CrossDomainRecommender, AutoEncoderMixin):
    r"""FFT
    ...
    def __init__(self, config, dataset):
        super(MacridDAE9, self).__init__(config, dataset)
    ...
    def forward(self, rating_matrix, user=None):
        ...
        for k in range(self.kfac):
            ...
            x_k = self.fft_enhance(rating_matrix, cates_k, noise=True)
            # x_k = rating_matrix
            h = self.encoder(x_k)
    ...
    def calculate_loss(self, interaction):
        ...
        return (ce_loss)
        return (ce_loss, self.lmd_disen * self.cl_loss)
