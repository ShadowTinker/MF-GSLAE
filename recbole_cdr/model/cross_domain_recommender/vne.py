...
class VNE(CrossDomainRecommender):
    ...
    def __init__(self, config, dataset):
        super(VNE, self).__init__(config, dataset)
    ...
    def calculate_loss(self, interaction):
        ...
        vne_loss = (
            self.get_vne(torch.cat([source_u_embeddings, target_u_embeddings])) + 
            self.get_vne(torch.cat([source_i_pos_embeddings, target_i_pos_embeddings]))
        )
        vne_loss = (
            self.get_vne(self.user_embedding.weight) + 
            self.get_vne(self.item_embedding.weight)
        )
        losses.append(self.beta * vne_loss)
