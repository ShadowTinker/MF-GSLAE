...
class MacridDAE37(CrossDomainRecommender, AutoEncoderMixin):
    r"""(27) + linear projection not mask

    """
    input_type = InputType.PAIRWISE

    def __init__(self, config, dataset):
        super(MacridDAE37, self).__init__(config, dataset)
    ...
    def forward(self, source_user=None, target_user=None):
        ...
        if source_user != None:
            ...
            for k in range(self.kfac):
                ...
                # z_k_source = F.normalize(self.proj_source(z_k_source))
            ...
            logits_source = sum(self.h_source_list) @ items_source.T / self.tau
        else:
            logits_source = None
        ...
        if target_user != None:
            ...
            for k in range(self.kfac):
                ...
                # z_k_target = F.normalize(self.proj_target(z_k_target))
            ...
            logits_target = sum(self.h_target_list) @ items_target.T / self.tau
        else:
            logits_target = None
