...
class MacridDAE26(CrossDomainRecommender, AutoEncoderMixin):
    r"""FFT + loss calculation (18) + intent structure

    """
    input_type = InputType.PAIRWISE

    def __init__(self, config, dataset):
        super(MacridDAE26, self).__init__(config, dataset)
        ...
        # self.intent_graph = nn.Parameter(torch.ones(self.kfac, self.kfac) / 20 + torch.eye(self.kfac) / 2) # we learn a sysmetric graph
        self.intent_graph = nn.Parameter(torch.ones(self.kfac, self.kfac) / 20, requires_grad=True)
        # self.intent_graph = nn.Parameter(torch.ones(self.kfac, self.kfac), requires_grad=True) # we learn a sysmetric graph
        ...
        # nn.init.xavier_normal_(self.intent_graph)
    ...
    def get_intent(self):
        def get_intent_graph():
            # if self.training:
            #     gumbels = (
            #         -torch.empty_like(self.intent_graph, memory_format=torch.legacy_contiguous_format).exponential_().log()
            #     ) / 10
            #     gumbels = (self.intent_graph + gumbels) / 1  # ~Gumbel(logits,tau)
            #     g = gumbels.softmax(-1)
            #     g
            # else:
            #     g = F.softmax(self.intent_graph, -1)
            g = F.relu(self.intent_graph)
            # g = F.normalize(g)
            g = g + g.T
            # degree = g.sum(1, keepdim=True).pow(-0.5).nan_to_num_(0, 0, 0)
            # g = degree * g * degree.T
            return g

        def lightgcn(g, emb, n_layer):
            emb_list = [emb]
        ...
        g = get_intent_graph()
        cores = lightgcn(g, self.k_embedding.weight, 1)
