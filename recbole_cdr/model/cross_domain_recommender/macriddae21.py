...
class MacridDAE21(CrossDomainRecommender, AutoEncoderMixin):
    r"""(18) + Mask
    ...
    def __init__(self, config, dataset):
        super(MacridDAE21, self).__init__(config, dataset)
        ...
        # nn.init.normal_(self.mask_source.weight, 0, 0.1)
        # nn.init.normal_(self.mask_target.weight, 0, 0.1)
    ...
    def factor_selection(self, h, mask):
        out = (h * mask.unsqueeze(0)).sum(1)
    ...
    def forward(self, source_user=None, target_user=None):
        ...
        if source_user != None:
            ...
            for k in range(self.kfac):
                ...
                probs_source_list.append(probs_k)
            probs_source = torch.stack(probs_source_list, dim=1)
            if self.mask_on:
                if self.training:
                    gumbels_noise = (
                        -torch.empty_like(
                        torch.zeros(1, self.kfac),
                        memory_format=torch.legacy_contiguous_format,
                        device=self.device
                    ).exponential_().log()
                    ) / 20
                    mask = (self.mask_source.weight + gumbels_noise).unsqueeze(-1)
                else:
                    mask = self.mask_source.weight.unsqueeze(-1)
                mask = torch.clamp(mask, -0.2, 0.2)
                mask = self.binary(mask)
                probs_source = probs_source * self.binary(mask)
            probs_source = probs_source.sum(1)
        ...
        if target_user != None:
            ...
            for k in range(self.kfac):
                ...
                probs_k = torch.exp(logits_k)
                probs_target_list.append(probs_k)
            probs_target = torch.stack(probs_target_list, dim=1)
            if self.mask_on:
                if self.training:
                    gumbels_noise = (
                        -torch.empty_like(
                    ...
                    mask = (self.mask_target.weight + gumbels_noise).unsqueeze(-1)
                else:
                    mask = self.mask_target.weight.unsqueeze(-1)
                mask = torch.clamp(mask, -0.2, 0.2)
                mask = self.binary(mask)
                probs_target = probs_target * mask
            probs_target = probs_target.sum(1)
