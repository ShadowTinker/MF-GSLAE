...
class MacridDAE35(CrossDomainRecommender, AutoEncoderMixin):
    r"""(30) learnbale mask + item-item graph reconstruction

    """
    input_type = InputType.PAIRWISE

    def __init__(self, config, dataset):
        super(MacridDAE35, self).__init__(config, dataset)
        ...
        # init_mask_source = torch.concat([torch.ones(3, device=self.device), torch.ones(1, device=self.device)])
        # init_mask_target = torch.concat([torch.ones(1, device=self.device), torch.ones(3, device=self.device)])
        # self.mask_source.weight.data.copy_(-0.1 * init_mask_source)
        # self.mask_target.weight.data.copy_(-0.1 * init_mask_target)
    ...
    def forward(self, source_user=None, target_user=None):
        ...
        if source_user != None:
            ...
            for k in range(self.kfac):
                ...
                local_user_graph = F.normalize(x_k_source) @ F.normalize(x_k_source).T
                graph_list.append(local_user_graph)
                ...
                # h_source = h_source + local_user_graph @ h_source
                ...
                # graph decoder
                # local_user_graph_rec = z_k_source @ z_k_source.T
                # self.graph_rec_loss_source += -(F.log_softmax(F.relu(local_user_graph_rec), 1) * local_user_graph).sum(1).mean()
                
            # mask = torch.stack(self.h_source_list, dim=0)
            # mask = self.mask_generator(mask)
            # probs_source = (torch.stack(probs_source, dim=0) * mask).sum(0)
            if self.if_mask:
                mask_source = self.binary(self.mask_source.weight).T.unsqueeze(-1)
                # mask_source = self.binary(self.mask_source.weight, self.source_mask_budget).T.unsqueeze(-1)
        ...
        if target_user != None:
            ...
            for k in range(self.kfac):
                ...
                # h_target = h_target + local_user_graph @ h_target
                ...
                # graph decoder
                # local_user_graph_rec = z_k_target @ z_k_target.T
                # self.graph_rec_loss_target += -(F.log_softmax(F.relu(local_user_graph_rec), 1) * local_user_graph).sum(1).mean()

            # mask = torch.stack(self.h_target_list, dim=0)
            # mask = self.mask_generator(mask)
            # probs_target = (torch.stack(probs_target, dim=0) * mask).sum(0)
            if self.if_mask:
                mask_target = self.binary(self.mask_target.weight).T.unsqueeze(-1)
                # mask_target = self.binary(self.mask_target.weight, self.target_mask_budget).T.unsqueeze(-1)
    ...
    def covariance_reg(self):
        ...
        L = F.normalize(L)
        covariance = L @ L.T
        loss = off_diagonal(covariance).pow_(2).sum() / self.kfac
        return loss

    # def cov_disen(self, emb):
    #     # emb: [N, K, D]
    #     def off_diagonal(x):
    #         b, n, m = x.shape
    #         assert n == m
    #         return x.flatten(-2, -1)[:, :-1].view(b, n - 1, n + 1)[:, :, 1:].flatten()
    #     emb = emb - emb.mean(-1, keepdim=True)
    #     cov = torch.bmm(emb, emb.transpose(1, 2))
    #     loss = off_diagonal(cov).pow_(2).sum(-1).sum()
    #     return loss
    ...
    def cov_disen(self, emb):
        ...
        def off_diagonal(x):
            ...
            assert n == m
            return x.flatten()[:-1].view(n - 1, n + 1)[:, 1:].flatten()
        emb = emb.flatten(0, 1) # [NK, D]
        emb = emb - emb.mean(-1, keepdim=True)
        cov = emb.T @ emb / emb.shape[0] / self.kfac
        loss = off_diagonal(cov).pow_(2).sum() / self.lat_dim
        return loss
    ...
    def mutual_information(self, emb):
        # emb: [N, K, D]
        loss = 0
        for k in range(self.kfac):
            for j in range(self.kfac):
                if k == j:
                    continue
                loss += InfoNCE(emb[:, k], emb[:, j])
        return loss
    ...
    def calculate_loss(self, interaction):
        ...
        # compactness_loss += self.compactness(torch.stack(self.h_source_list, dim=1))
        # compactness_loss += self.compactness(torch.stack(self.h_target_list, dim=1))

        cov_disen = torch.tensor(0., device=self.device)
        # cov_disen += self.cov_disen(torch.stack(self.h_source_list, dim=1))
        # cov_disen += self.cov_disen(torch.stack(self.h_target_list, dim=1))
        ...
        # )
        # mask_reg = (
        #     F.relu(self.mask_source.weight) + F.relu(self.mask_target.weight)
        # ).sum()
        mask_reg = -(self.mask_source.weight ** 2 + self.mask_target.weight ** 2).mean()

        mutual_info = torch.tensor(0., device=self.device)
        mutual_info += self.mutual_information(torch.stack(self.h_source_list, dim=1))
        mutual_info += self.mutual_information(torch.stack(self.h_target_list, dim=1))

        # Graph rec loss

        return (ce_loss_source, ce_loss_target, 0e-3 * mutual_info)
