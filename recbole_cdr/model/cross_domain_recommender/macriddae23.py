...
class MacridDAE23(CrossDomainRecommender, AutoEncoderMixin):
    ...
    def __init__(self, config, dataset):
        super(MacridDAE23, self).__init__(config, dataset)
        ...
        self.k_embedding = nn.Parameter(torch.zeros(self.lat_dim, self.lat_dim, self.kfac))
        ...
        nn.init.xavier_normal_(self.k_embedding)
    ...
    def forward(self, source_user=None, target_user=None):
        cores = F.normalize(self.k_embedding, dim=0)
        ...
        if source_user != None:
            ...
            cates_logits = (rating_matrix_source @ items @ cores / self.tau).permute(1, 0, 2)
            ...
            for k in range(self.kfac):
                ...
                x_k_source = rating_matrix_source
                h_source = self.encoder(x_k_source)
                h_source = h_source * cates_k
        ...
        if target_user != None:
            ...
            cates_logits = (F.normalize(rating_matrix_target @ items) @ cores / self.tau).permute(1, 0, 2)
            cates = self.get_cates(cates_logits)
    ...
    def calculate_loss(self, interaction):
        ...
        return (ce_loss_source, ce_loss_target, self.lmd_disen * compactness_loss, self.lmd_disen * reg_loss)
