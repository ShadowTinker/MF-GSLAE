...
class MacridDAE5(CrossDomainRecommender, AutoEncoderMixin):
    ...
    def __init__(self, config, dataset):
        super(MacridDAE5, self).__init__(config, dataset)
        ...
        self.user_gate = MLPLayers([self.lat_dim, 1], dropout=0., activation=nn.Tanh)
    ...
    def vicerg_variance(self, h):
        return torch.mean(F.relu(1 - torch.sqrt(h.var(dim=0) + 1e-04)))
    
    def vicreg_covariance(self, h):
        def get_off_diagonal_elements(M):
            res = M.clone()
            res.diagonal(dim1=-1, dim2=-2).zero_()
            return res
        h = h - h.mean(dim=0)
        cov_z_a = (h.T @ h) / (h.shape[0] - 1)
        cov_loss = (get_off_diagonal_elements(cov_z_a)).pow_(2).sum() / h.shape[-1]
        return cov_loss
    ...
    #     # rating_matrix = self.sprase_drop(self.sparse_norm_rating_matrix)
    ...
    #         # cates_k = cates[:, k]
    #         # encoder
    #         # x_k = torch.sparse.mm(rating_matrix, torch.diag(cates_k))
    #         x_k = torch.sparse.mm(rating_matrix, torch.diag(cates_k.squeeze()).to_sparse())
    #         h = torch.sparse.mm(x_k, self.encoder.weight.T) + self.encoder.bias
            
    #         h = self.get_gnn_embeddings(h)

    #         # decoder
    #         z_k = F.normalize(h, dim=1)[user]
    ...
    def forward(self, rating_matrix, user=None):
        ...
        cores = F.normalize(self.k_embedding.weight, dim=1)
        items = F.normalize(self.item_embedding.weight, dim=1)
        users = F.normalize(self.user_embedding.weight, dim=-1)[user]
        user_gate = self.user_gate(users).unsqueeze(-1)

        rating_matrix = F.normalize(rating_matrix)
        rating_matrix = F.dropout(rating_matrix, self.drop_out, training=self.training)

        cates_logits = torch.matmul(items, cores.transpose(0, 1)) / self.tau
        cates_logits = user_gate * cates_logits.unsqueeze(0)
