...
from recbole_cdr.model.layers import Encoder, Binarize, MyTrmEncoderLayer, MyTransformerEncoder
...
class MacridDAE15(CrossDomainRecommender, AutoEncoderMixin):
    r"""FFT + loss calculation

    """
    input_type = InputType.PAIRWISE

    def __init__(self, config, dataset):
        super(MacridDAE15, self).__init__(config, dataset)
        ...
        #     num_layers=1,
        # )
        transformer_encoder = MyTrmEncoderLayer(
            d_model=self.lat_dim,
            nhead=1,
            dim_feedforward=self.lat_dim * 4,
            dropout=0.5,
        ...
        self.transformer_layer = MyTransformerEncoder(
            encoder_layer=transformer_encoder,
            num_layers=1,
        )
    ...
    #     A_abs = A_abs * cates_k
    #     A_hat = A_abs * (torch.e ** (1j * A_pha))
    #     A_hat = torch.fft.ifft(A_hat).real
    #     return A_hat
    ...
    def forward(self, source_user=None, target_user=None):
        ...
        if source_user != None:
            ...
            for k in range(self.kfac):
                ...
                h_source, graph = self.transformer_layer(h_source)
        ...
        if target_user != None:
            ...
            for k in range(self.kfac):
                ...
                h_target, graph = self.transformer_layer(h_target)
                # _, h_target = self.factor_selection(h_target)
