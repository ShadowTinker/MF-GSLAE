...
class MacridVAE2(CrossDomainRecommender, AutoEncoderMixin):
    ...
    def __init__(self, config, dataset):
        super(MacridVAE2, self).__init__(config, dataset)
        ...
        self.encode_layer_dims = self.layers + [self.lat_dim * 2]
    ...
    def forward(self, rating_matrix_source, rating_matrix_target):
        ...
        cates_logits_target = torch.matmul(items_target, cores_target.transpose(0, 1)) / self.tau_target
        ...
        for k in range(self.kfac):
            ...
            # decoder
            z_k = F.normalize(z, dim=1)
            logits_k = torch.matmul(z_k, items_target.transpose(0, 1)) / self.tau_target
            probs_k = torch.exp(logits_k)
            probs_k = probs_k * cates_k
        ...
        return logits_source, logits_target, mulist, logvarlist
    ...
    def calculate_loss(self, interaction):
        ...
        z_source, z_target, mu, logvar = self.forward(rating_matrix_source, rating_matrix_target)
        ...
        if self.regs[0] != 0 or self.regs[1] != 0:
            return ce_loss_source + ce_loss_target + kl_loss * anneal + self.reg_loss()
        return (ce_loss_source, ce_loss_target, kl_loss)
    ...
    def full_sort_predict_source(self, interaction):
        ...
        scores, _, _, _ = self.forward(rating_matrix, self.rating_matrix_target[[0, 1]].to(self.device))
    ...
    def full_sort_predict_target(self, interaction):
        ...
        _, scores, _, _  = self.forward(self.rating_matrix_source[[0, 1]].to(self.device), rating_matrix)
