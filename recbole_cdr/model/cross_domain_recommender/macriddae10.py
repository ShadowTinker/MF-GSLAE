...
class MacridDAE10(CrossDomainRecommender, AutoEncoderMixin):
    r"""phase drop
    ...
    def __init__(self, config, dataset):
        super(MacridDAE10, self).__init__(config, dataset)
    ...
    def fft_division(self, A):
        A_fft = torch.fft.fft(A.T)
        A_abs, A_pha = torch.abs(A_fft), torch.angle(A_fft)
        A_pha = F.dropout(A_pha, 0.1, training=self.training)
        return A_abs, A_pha

    def fft_enhance(self, A_abs, A_pha, cates_k):
    ...
    def forward(self, rating_matrix, user=None):
        ...
        A_abs, A_pha = self.fft_division(rating_matrix)
        ...
        for k in range(self.kfac):
            ...
            x_k = self.fft_enhance(A_abs, A_pha, cates_k)
            # x_k = rating_matrix
            h = self.encoder(x_k)
