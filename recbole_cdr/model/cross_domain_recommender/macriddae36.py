...
class MacridDAE36(CrossDomainRecommender, AutoEncoderMixin):
    r"""(30) learnbale mask + bootstrap

    """
    input_type = InputType.PAIRWISE

    def __init__(self, config, dataset):
        super(MacridDAE36, self).__init__(config, dataset)
        ...
        # init_mask_source = torch.concat([torch.ones(5, device=self.device), -torch.ones(5, device=self.device)])
        # init_mask_target = torch.concat([-torch.ones(5, device=self.device), torch.ones(5, device=self.device)])
        ...
        # self.mask_source.weight.data.copy_(-1. * init_mask_source)
        # self.mask_target.weight.data.copy_(-1. * init_mask_target)
    ...
    def forward(self, source_user=None, target_user=None):
        ...
        if source_user != None:
            ...
            for k in range(self.kfac):
                ...
                h_source_res, graph = self.transformer_layer(F.normalize(h_source))
                h_source = h_source + h_source_res * (1 - self.alpha_)
        ...
        if target_user != None:
            ...
            for k in range(self.kfac):
                ...
                h_target_res, graph = self.transformer_layer(F.normalize(h_target))
                h_target = h_target + h_target_res * (1 - self.alpha_)
        ...
        self.alpha_ = self.alpha_ * self.alpha
