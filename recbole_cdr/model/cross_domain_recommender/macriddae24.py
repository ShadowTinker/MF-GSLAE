...
class MacridDAE24(CrossDomainRecommender, AutoEncoderMixin):
    r"""FFT + loss calculation (18) -> representation then decomposition by intent splitted by representation

    """
    input_type = InputType.PAIRWISE

    def __init__(self, config, dataset):
        super(MacridDAE24, self).__init__(config, dataset)
    ...
    def forward(self, source_user=None, target_user=None):
        ...
        if source_user != None:
            ...
            h_source_all = self.encoder(rating_matrix_source)
            cates_logits = F.normalize(h_source_all) @ cores.T / self.tau
        ...
        if target_user != None:
            ...
            h_target_all = self.encoder(rating_matrix_target)
            cates_logits = F.normalize(h_target_all) @ cores.T / self.tau
